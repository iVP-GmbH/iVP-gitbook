...
To complete a path it is either possible to press **Enter** or **double click on the last measurement point** set. With its completion, the path will also be saved as a permanent object that can be adjusted or deleted via the [path panel](../user-interface/path-panel.md).
...
Existing paths can be selected by **clicking on the path name** in the [2D-panel](../user-interface/the-2d-panel.md) **or the entry** **of the path** in the[ path panel](../user-interface/path-panel.md).
...
Alternatively it is possible to hide or show all lengths or angles by **right clicking** on the path entry in the [path panel](../user-interface/path-panel.md) and subsequently clicking on **"Edge Lengths"** or **"Angles"**.
...
Besides lengths and angles, areas are measurable as well. To activate this option, **right click on the path entry** in the path panel, followed by a click in **"Area"**.
...
Paths can be renamed at any time via the [path panel](../user-interface/path-panel.md). After **right clicking on the desired path** and choosing **"Rename"**, the name of the path will get a blue square as background. It can now be changed by **entering the new name**, followed by the **Enter**.
